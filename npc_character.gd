...
func _process(delta):
	...
	var base = skeleton.get_bone_global_pose_no_override(head_bone)
	head_angle = lerp_angle(head_angle, target_angle, 0.03)
	base.basis = Basis(Vector3(0, 1, 0), head_angle)
	skeleton.set_bone_global_pose_override(head_bone, base, 1.0, true)
